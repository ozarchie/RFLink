...
void setup() {
  ...
  Serial.print(F("20;00;Nodo RadioFrequencyLink - RFLink Gateway V1.1a - "));
...
void loop() {
  ...
  while(true) {
    ...
    // SERIAL: *************** see if there is data ready on the serial port **********************
    if(Serial.available()) {
        ...
        if(Serial.available()) {
          ...
          if(isprint(SerialInByte))
            if(SerialInByteCounter < (INPUT_COMMAND_SIZE - 1))
              InputBuffer_Serial[SerialInByteCounter += 1] = SerialInByte;      // store in buffer
              
          if(SerialInByte == '\n') {                                            // new line character
            InputBuffer_Serial[SerialInByteCounter] = 0;                        // serieel data is complete
            //Serial.print("20;incoming;"); 
            //Serial.println(InputBuffer_Serial); 
            if (strlen(InputBuffer_Serial) > 7){                                // need to see minimum 8 characters on the serial port
               // 10;....;..;ON; 
               if (strncmp (InputBuffer_Serial,"10;",3) == 0) {                 // Command from Master to RFLink
                  ...
                  if (strcasecmp(InputBuffer_Serial+3, "PING;") == 0) {
                     sprintf(InputBuffer_Serial, "20;%02X;PONG;", PKSequenceNumber += 1);
                     Serial.println(InputBuffer_Serial); 
                  } else
                  ...
                  if (strncasecmp(InputBuffer_Serial+3, "RFDEBUG=O", 9) == 0) {
                     if (InputBuffer_Serial[12] == 'N' || InputBuffer_Serial[12] == 'n' ) {
                        ...
                        sprintf(InputBuffer_Serial, "20;%02X;RFDEBUG=ON;", PKSequenceNumber += 1);
                     } else {
                        RFDebug = false;                                          // full debug off
                        sprintf(InputBuffer_Serial, "20;%02X;RFDEBUG=OFF;", PKSequenceNumber += 1);
                     }
                     Serial.println(InputBuffer_Serial); 
                  } else                 
                  if (strncasecmp(InputBuffer_Serial+3, "RFUDEBUG=O", 10) == 0) {
                     if (InputBuffer_Serial[13] == 'N' || InputBuffer_Serial[13] == 'n') {
                        ...
                        QRFDebug = false;                                         // undecoded debug off
                        RFDebug = false;                                          // full debug off
                        sprintf(InputBuffer_Serial,"20;%02X;RFUDEBUG=ON;",PKSequenceNumber += 1);
                     } else {
                        RFUDebug = false;                                         // undecoded debug off
                        sprintf(InputBuffer_Serial, "20;%02X;RFUDEBUG=OFF;", PKSequenceNumber += 1);
                     }
                     Serial.println(InputBuffer_Serial); 
                  } else                 
                  if (strncasecmp(InputBuffer_Serial+3,"QRFDEBUG=O",10) == 0) {
                     if (InputBuffer_Serial[13] == 'N' || InputBuffer_Serial[13] == 'n') {
                        ...
                        RFUDebug = false;                                         // undecoded debug off 
                        RFDebug = false;                                          // full debug off
                        sprintf(InputBuffer_Serial, "20;%02X;QRFDEBUG=ON;", PKSequenceNumber += 1);
                     } else {
                        QRFDebug = false;                                         // undecoded debug off
                        sprintf(InputBuffer_Serial, "20;%02X;QRFDEBUG=OFF;", PKSequenceNumber += 1);
                     }
                     Serial.println(InputBuffer_Serial); 
                  } else                 
                  if (strncasecmp(InputBuffer_Serial+3, "VERSION",7) == 0) {
                      sprintf(InputBuffer_Serial, "20;%02X;VER=1.1;REV=%02x;BUILD=%02x;" ,PKSequenceNumber += 1,REVNR, BUILDNR);
                      Serial.println(InputBuffer_Serial); 
                  } else {
                     // -------------------------------------------------------
            ...
            if (ValidCommand != 0) {
               if (ValidCommand==1) {
                  sprintf(InputBuffer_Serial, "20;%02X;OK;", PKSequenceNumber += 1);
                  Serial.println( InputBuffer_Serial ); 
               } else {
                  sprintf(InputBuffer_Serial, "20;%02X;CMD UNKNOWN;", PKSequenceNumber += 1); // Node and packet number 
                  Serial.println( InputBuffer_Serial );
               }   
            }
